...
import com.ms.util.internal.SocketUtils;
...
public class ServerBootstrap<C extends Channel> {
    ...
    private ChannelFuture doBind(SocketAddress localAddress) {
        ...
        // 在 initAndRegister() 里面 regFuture 注册 有可能是异步
        ...
        if (regFuture.isDone()) {
            // 如果完成的话，则再执行绑定端口
            // initAndRegister (创建serverSocketChannel -> 注册到selector上)
    ...
    final ChannelFuture initAndRegister() {
        ...
        // 做了两件事 ： 创建了原生的 serverSocketChannel 然后设置为非阻塞
        channel = channelFactory.newChannel();
        //这里是异步注册的，一般来说，bossGroup设置的都是一个线程。
        // 这里做了两件事  注册到bossGroup里面nioEventLoop的selector上面；注册了接收事件
