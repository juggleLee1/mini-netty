...
public class Bootstrap<C extends Channel> {
    ...
    public Bootstrap channel(Class<? extends C> channelClass) {
        // 产生 channel 的工厂 根据传入的channelClass（NioServerSocketChannel 或者 NioSocketChannel）
        this.channelFactory = new ReflectiveChannelFactory<C>(channelClass);
        return this;
    }

    /**
     * 这里本来还有一个同名方法，参数是SocketAddress类型的，可以去源码中看一下
    ...
    private ChannelFuture doResolveAndConnect(final SocketAddress remoteAddress, final SocketAddress localAddress) {
        //这里的逻辑和serverbootstarp一样，但是在这里又要写一遍该方法，现在是不是发现，
        //如果bootstarp和serverbootstarp有一个
