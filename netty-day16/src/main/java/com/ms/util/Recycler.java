...
import com.ms.util.concurrent.FastThreadLocal;
...
import static com.ms.util.internal.MathUtil.safeFindNextPositivePowerOfTwo;
...
public abstract class Recycler<T> {
    ...
    /**
     * @Author: PP-jessica
     * @Description:handle实际上就是从对象池中获得对象，而handler中的一个成员变量是真正我们需要的对象
     */
    ...
    /**
     * @Author: PP-jessica
     * @Description:该属性是是用来给线程提供特殊化id的，这个id是给生产对象的线程和回收对象的线程赋值的，
     * 并且就用赋的这个值区分这两种类型的线程
     */
    ...
    /**
     * @Author: PP-jessica
     * @Description:这个属性被static和final修饰，这说明该属性一旦被初始化就不会被改变，并且只初始化一次
    ...
    /**
     * @Author: PP-jessica
     * @Description:每一个线程对应的stack可以存储的数据的最大个数，stack存储数据的是一个数组容器，默认最大容量为4096
     * 毕竟对象池不能无限扩大。
     */
    ...
    /**
     * @Author: PP-jessica
     * @Description:这是一个辅助属性，帮助计算所有帮忙回收对象的线程能够为原来获取对象的线程能够回收的对象的最大值
     * 这个也是在限制对象池的容量，不能让它无限扩大，默认值为2。
     */
    private static final int MAX_SHARED_CAPACITY_FACTOR;
    /**
     * @Author: PP-jessica
     * @Description:每一个线程最多可以帮助几个线程回收对象。默认值是cpu的核数乘以2
     * 其实就是默认的工作组线程的数量
    ...
    /**
     * @Author: PP-jessica
     * @Description:link链表中每一个link节点使用数组存储待回收的对象，这个数组的默认容量为16
    ...
    /**
     * @Author: PP-jessica
     * @Description:对象被回收的比例。默认是8比1。比如一个线程从线程池也就是它的本地stack中得到了8个对象，那么就把第8个对象回收进来
    ...
    /**
     * @Author: PP-jessica
     * @Description:这个就是对象池了，说是对象池，实际上就是每个线程私有的对象池的入口，也就是stack的入口
    ...
    /**
     * @Author: PP-jessica
     * @Description:对象池的重点方法，我称它为每个线程的对象池的入口方法，就是通过这个方法，每个线程得到了
    ...
    /**
     * @Author: PP-jessica
     * @Description:通过Recycler类来回收对象的方法，已经被废弃了
     */
    @Deprecated
    ...
    /**
     * @Author: PP-jessica
     * @Description:该方法是为了得到线程的对象池的容量，也就是stack中数组的容量
    ...
    /**
     * @Author: PP-jessica
     * @Description:该方法是为了得到线程的对象池stack中数组存储元素的个数
     */
    ...
    /**
     * @Author: PP-jessica
     * @Description:之前提到的需要在池化的类中实现的抽象方法
     */
    protected abstract T newObject(Handle<T> handle);

    /**
     * @Author: PP-jessica
     * @Description:Recycler类中的定义的内部接口，实现也是在本类中实现的。
     */
    ...
    /**
     * @Author: PP-jessica
     * @Description:接口的静态实现类
     */
    static final class DefaultHandle<T> implements Handle<T> {
        /**
         * @Author: PP-jessica
         * @Description:这个属性就是用来标志这个对象暂时被哪个线程回收了。这里可以看到，当一个线程从自己的对象池中获得了
        ...
        /**
         * @Author: PP-jessica
         * @Description:这个属性就是用来标记从对象池中获得的对象最终被哪个线程回收了。这个属性其实对应的就是获得对象的那个线程
    ...
    /**
     * @Author: PP-jessica
     * @Description:WeakOrderQueue队列是一个重要的容器。它的由来其实很简单。
    ...
    private static final class WeakOrderQueue {
        ...
        /**
         * @Author: PP-jessica
         * @Description:link就是WeakOrderQueue对象中的每一个节点。直接一点，就是WeakOrderQueue本身就是一个队列，这个队列
         * 是由link对象构成的，而每个线程的WeakOrderQueue又构成了一个队列。自身线程从其他线程取回自己的对象时，会先从第一个WeakOrderQueue
         * 节点中查看WeakOrderQueue中的link节点中有没有要被回收的对象，如果有就把对象转移到自己的stack中去
         */
        ...
        /**
         * @Author: PP-jessica
         * @Description:WeakOrderQueue对象内部的link头节点。设置这个头节点的目的很简单，
        ...
        static final class Head {
            ...
            /**
             * @Author: PP-jessica
             * @Description:该方法和垃圾回收有关，实际上做的工作就是在没清空一个link对象之后，给availableSharedCapacity加上
            ...
            /**
             * @Author: PP-jessica
             * @Description:每释放一个link节点，就为该属性加上16，相反，每创建一个link节点，就要为该属性减16
             */
            ...
            /**
             * @Author: PP-jessica
             * @Description:为availableSharedCapacity减16
             */
            ...
            /**
             * @Author: PP-jessica
             * @Description:availableSharedCapacity是2048个，初始化好的。创建了一个队列，添加了一个link节点，所以要先减去16个容量
       ...
       /**
        * @Author: PP-jessica
        * @Description:分配一个新的WeakOrderQueue对象的方法
        */
        ...
        /**
         * @Author: PP-jessica
         * @Description:判断WeakOrderQueue对象中是否还有可被回收的对象。
    ...
    static final class Stack<T> {
        ...
        /**
         * @Author: PP-jessica
         * @Description:这个就是从stack对象池获取缓存对象的方法，
        ...
        /**
         * @Author: PP-jessica
         * @Description:该方法只有返回false，才能使对象被真正回收，回收比例确认
         */
